...
export default class AutogridQuery {
	...
	getLastBase(items) {
		return items.filter(this.checkIsBaseItem).reduce((v, item) => item['value'], NaN);
	}
	...
	dataCollection(item, propName) {
		let { query, value } = this.getQueryAndPropCSS( item['value'], item['min'], item['max'], propName, this.otherData );

		// заполняем
		if( !this.QUERY_AND_PROPS_CSS[query] ) this.QUERY_AND_PROPS_CSS[query] = [];
		this.QUERY_AND_PROPS_CSS[query].push(value);
	}

	queryAndProps_toStyleCSS() {
		for( let key in this.QUERY_AND_PROPS_CSS ) {
			this.STYLE_CSS += key + `{${ this.selector }{${ this.QUERY_AND_PROPS_CSS[key].join('') }}}`;
		}
